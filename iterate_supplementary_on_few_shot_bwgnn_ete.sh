...
for seed in "${seeds[@]}"; do
    for target_dataset in "${target_datasets[@]}"; do
        ...
        do
            ...
            do
                python src/exec.py --general.func ete --general.seed ${seed} --general.save_dir "bwgnn_storage/balanced_few_shot_ete/5_shot" --general.few_shot $few_shot  --data.node_feature_dim 100 --data.name $target_dataset --data.supervised.ratios 0.01,0.19,0.8 --model.backbone.model_type ${backbone} --model.saliency.model_type "none" --ete.learning_rate $lr --ete.batch_size $bs
            done 
        done    
    done
done