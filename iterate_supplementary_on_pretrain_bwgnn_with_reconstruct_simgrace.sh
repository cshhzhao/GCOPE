...
for target_dataset in "${target_datasets[@]}"; do
    ...
    echo $target_dataset
    echo "bwgnn_storage_simgrace/reconstruct/${source_dataset_str}_pretrained_model.pt"

    # 执行 exec.py 文件并传递参数
    python src/exec.py --config-file pretrain_bwgnn_simgrace.json --general.save_dir "bwgnn_storage_simgrace/reconstruct" --general.reconstruct 0.2 --data.name "${source_dataset_str}" --pretrain.split_method ${split_method}
    ...
    do
        ...
        do
            python src/exec.py --general.func adapt  --general.save_dir "bwgnn_storage_simgrace/balanced_few_shot_fine_tune_backbone_with_rec" --general.few_shot $few_shot  --general.reconstruct 0.0 --data.node_feature_dim 100 --data.name $target_dataset --adapt.method finetune --data.supervised.ratios 0.1,0.1,0.8 --model.backbone.model_type ${backbone} --model.saliency.model_type "none" --adapt.method finetune --adapt.pretrained_file "bwgnn_storage_simgrace/reconstruct/${source_dataset_str}_pretrained_model.pt" --adapt.finetune.learning_rate $lr --adapt.batch_size $bs --adapt.finetune.backbone_tuning $backbone_tuning
        done 
    done
done