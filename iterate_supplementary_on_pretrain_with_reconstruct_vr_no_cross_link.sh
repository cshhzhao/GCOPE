...
# 执行参数都在pretrain_no_cross_link_with_rec.json文件中，cross_link_ablation=True，不会再虚点之间添加边。
...
for target_dataset in "${target_datasets[@]}"; do
    ...
    echo $target_dataset
    echo "storage/vr_no_cross_link_with_rec/${source_dataset_str}_pretrained_model.pt"    

    python src/exec.py --config-file pretrain_no_cross_link_with_rec.json --general.save_dir "storage/vr_no_cross_link_with_rec" --general.reconstruct 0.2 --data.name "${source_dataset_str}"
