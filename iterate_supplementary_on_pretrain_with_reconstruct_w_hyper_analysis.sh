...
# target dataset选择citeseer
# reconstruct 取值从0~1.0的结果 (0.2的已经有了)
...
for target_dataset in "${target_datasets[@]}"; do
    ...
    echo $target_dataset
    echo "storage/reconstruct_w_hyper_analysis/lambda_10/${source_dataset_str}_pretrained_model.pt"    

    python src/exec.py --config-file pretrain_w_hyper_analysis.json --general.save_dir "storage/reconstruct_w_hyper_analysis/lambda_10" --data.name "${source_dataset_str}" --pretrain.split_method ${split_method}
    ...
    do
        ...
        do
            python src/exec.py --general.func adapt  --general.save_dir "storage/balanced_few_shot_fine_tune_backbone_with_rec_w_hyper_analysis/lambda_10" --general.few_shot $few_shot  --general.reconstruct 0.0 --data.node_feature_dim 100 --data.name $target_dataset --adapt.method finetune --data.supervised.ratios 0.1,0.1,0.8 --model.backbone.model_type ${backbone} --model.saliency.model_type "none" --adapt.method finetune --adapt.pretrained_file "storage/reconstruct_w_hyper_analysis/lambda_10/${source_dataset_str}_pretrained_model.pt" --adapt.finetune.learning_rate $lr --adapt.batch_size $bs --adapt.finetune.backbone_tuning $backbone_tuning
        done 
    done
done