...
# target_datasets=("cora" "citeseer" "pubmed" "computers" "photo")
# target_datasets=("cornell" "wisconsin" "texas" "chameleon" "squirrel")
target_datasets=("citeseer" "cora" "pubmed" "wisconsin" "texas" "cornell" "chameleon" "squirrel" "computers" "photo")
...
for seed in "${seeds[@]}"; do
    ...
    for target_dataset in "${target_datasets[@]}"; do
        ...
        do
            ...
            do
                python src/exec.py --general.func adapt --general.seed ${seed} --general.save_dir "storage/supervised_fine_tune_backbone_with_isolated" --general.few_shot $few_shot --general.reconstruct 0.0  --data.node_feature_dim 100 --data.name $target_dataset --adapt.method finetune --data.supervised.ratios 0.1,0.1,0.8 --model.backbone.model_type ${backbone} --model.saliency.model_type "none" --adapt.method finetune --adapt.pretrained_file "storage/isolated_pretrain/best_loss/${source_dataset_str}_pretrained_model.pt" --adapt.finetune.learning_rate $lr --adapt.batch_size $bs --adapt.finetune.backbone_tuning $backbone_tuning
            done 
        done    
    done
done